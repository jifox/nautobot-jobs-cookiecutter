...
```shell
pip install nautobot-jobs
```

> The plugin is compatible with Nautobot 1.2.0 and higher

To ensure Nautobot-Jobs is automatically re-installed during future upgrades, create a file named `local_requirements.txt` (if not already existing) in the Nautobot root directory (alongside `requirements.txt`) and list the `nautobot-jobs` package:

```no-highlight
# echo nautobot-jobs >> local_requirements.txt
```
...
PLUGINS = ["nautobot_chatops", "nautobot-jobs"]
...
- `project_name`: the default docker compose project name (default: nautobot-jobs)
...
Using PyInvoke these configuration options can be overridden using [several methods](http://docs.pyinvoke.org/en/stable/concepts/configuration.html). Perhaps the simplest is simply setting an environment variable `INVOKE_NAUTOBOT-JOBS_VARIABLE_NAME` where `VARIABLE_NAME` is the variable you are trying to override. The only exception is `compose_files`, because it is a list it must be overridden in a yaml file. There is an example `invoke.yml` in this directory which can be used as a starting point.
...
Each command can be executed with `invoke <command>`. Environment variables `INVOKE_NAUTOBOT-JOBS_PYTHON_VER` and `INVOKE_NAUTOBOT-JOBS_NAUTOBOT_VER` may be specified to override the default versions. Each command also has its own help `invoke <command> --help`
