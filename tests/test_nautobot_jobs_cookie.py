"""Test Bake Python Project."""
import datetime

import pytest

PYTHON_COOKIE_OUTPUT = [
...
def test_bake_project(cookies, python_cookie_dir):
    """Test Bake Project."""
    result = cookies.bake(
        template=python_cookie_dir,
        extra_context={"project_name": "helloworld"},
    )
...
def test_year_compute_in_license_file(cookies, python_cookie_dir):
    """Test year compute in license file."""
    result = cookies.bake(template=python_cookie_dir)
    license_file_path = result.project_path.joinpath("LICENSE")
    now = datetime.datetime.now()
    assert str(now.year) in license_file_path.read_text()


def test_cookie_output_dir(cookies, python_cookie_dir):
    """Test cookie output dir."""
    result = cookies.bake(template=python_cookie_dir)
    assert set(PYTHON_COOKIE_OUTPUT) == {dirf.name for dirf in result.project_path.iterdir()}
...
def test_bake_selecting_license(cookies, python_cookie_dir, license_info):
...
def test_bake_not_open_source(cookies, python_cookie_dir):
