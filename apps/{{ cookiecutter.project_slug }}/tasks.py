...
def print_command(command, env=None):
    r"""
    >>> command = "docker buildx build . --platform linux/amd64 --target final --load -f ./docker/Dockerfile --build-arg PYTHON_VER={{ cookiecutter.python_version }} -t networktocode/nautobot-py{{ cookiecutter.python_version }}:local --no-cache"
...
def run_dbcmd(context, command):
    """Execute command in database container.

    Args:
        context (obj): Used to run specific commands
        command (str): bash command to execute in db container
    """
    print("Executing command in db container...")

    docker_compose(context, f"exec -T db bash -c {quote(command)}")
